...
def execute(filters=None):
	...
	# Dictionary to store the aggregated sums for each key
	aggregated_data = {}
	...
	for item in data:
		...
		if key not in aggregated_data:
			aggregated_data[key] = [item[1], item[2], item[2] - item[1]]
		else:
			aggregated_data[key][0] += item[1]
			aggregated_data[key][1] += item[2]
			aggregated_data[key][2] += item[2] - item[1]

	# Create a new list with the aggregated values
	result = [[key] + values for key, values in aggregated_data.items()]	
...
def get_columns(filters):
	return [
		{
			"fieldname": "order_created_by",
			"label": _("Order Created By"),
			"fieldtype": "Data",
			"width": 230,
		},
		# {
		...
		{"fieldname": "debit", "label": _("Debit"), "fieldtype": "Currency", "width": 200},
		{"fieldname": "credit", "label": _("Credit"), "fieldtype": "Currency", "width": 200},
		{"fieldname": "total", "label": _("Total(C-D)"), "fieldtype": "Currency", "width": 200},
...
def get_invoice_posting_date_map(filters):
	...
	for t in frappe.db.sql("select name, posting_date, due_date, order_created_by, order_created_user_name from `tab{0}`".format(dt), as_dict=1):
